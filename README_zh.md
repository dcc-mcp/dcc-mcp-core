...
dcc-mcp-core 来自两类长期实践：DCC 工具开发与部署，以及高强度的 Agent 使用。

最直接的 DCC Agent Demo，是让模型临时写一段 `mayapy`、`hython` 或 Blender
Python，然后马上执行。Demo 可以这样做，生产不能靠这个。

影视和游戏生产追求的是可重复，不是每次调用都“抽卡”。让模型现场写代码，效果会随
模型能力、上下文和提示词变化；同一段业务逻辑下次还要重新生成，也会重复消耗 Token。
与此同时，每个 DCC adapter 都要重新处理通信、主线程、参数校验、进程生命周期、实例
路由和日志。这些才是集成里最费时间、也最容易踩坑的部分。

dcc-mcp-core 把这部分公共工程收进一个可复用框架：

| 层次 | 可以直接复用的能力 |
|---|---|
| 接口接入 | MCP/REST endpoint、Host RPC/IPC、类型化 schema、resources、prompts、结构化结果 |
| DCC 运行时 | 主线程亲和、readiness、多实例路由、异步 job、取消、checkpoint、workflow、artefact 交接、UI Control |
| Skill 交付 | `SKILL.md` 包、渐进式发现、lint/schema 校验、hot reload、状态持久化、marketplace、项目/团队作用域 |
| 生产运维 | CLI、gateway、Admin UI、权限策略、audit、trace、logs、metrics、health check、VRS replay |

dcc-mcp-core 不做 Agent，它做的是 **Agent 操作 DCC 的基础设施**，也不绑定用户必须
选择哪个 Agent。Agent 会换，模型会升级，但工作室的 DCC 接口、权限边界和 pipeline
经验仍需要长期维护。框架把这些沉淀成可以复用的工程资产。
...
为此，dcc-mcp-core 提供了类似 Computer Use 的 **UI Control**。它把精确窗口截图、
可用时的语义控件、作用域操作、等待、录制、策略检查、审计和结果验证放进同一套流程。
能用原生 Skill/API 时仍然优先使用；UI Control 让没有可靠接口的旧工具和纯界面流程
也能进入 Agent 控制面。下文的 DCC UI Control 技术章节说明了它的边界和工作方式。

### MCP 是入口，不是能力上限

名字里有 MCP，但框架不只等于 MCP。MCP 是我们复用的行业标准入口，不是能力上限。
只要宿主提供稳定的 Python、C++、HTTP、command port 或原生插件接口，就可以接入同一
套发现、执行、安全和运维契约，而不必再为每个 DCC 重新造一套轮子。

我们也不打算替代 DCC 厂商已经做好的能力。Unreal Engine 5.8 加入了
[实验性的官方 MCP 和 Toolset Registry](https://dev.epicgames.com/documentation/unreal-engine/unreal-mcp-in-unreal-editor)；
[`unreal-official-mcp`
Skill](https://github.com/dcc-mcp/dcc-mcp-unreal/blob/main/src/dcc_mcp_unreal/skills/unreal-official-mcp/SKILL.md)
可以启用并桥接这些官方 toolset，不重新分发 Epic 插件，也不改写官方工具名称和
schema。这样官方能力、DCC MCP 自己的能力和工作室内部工具，可以进入同一套 Agent
工作流。

### 为什么以 Skill 为生产单元

Skill 不是把脚本换一个目录存放。它把已经验证过的 pipeline 经验，变成有版本、有
schema、可测试、可分发的工具。

一个成本更低的模型从零编写场景逻辑可能不稳定，但如果它只需要选择一个说明清楚的
工具，再填写经过校验的参数，也能更稳定地完成任务。这样可以减少临场代码生成和
Token 消耗，也降低生产结果对模型强弱的依赖。
...
Gateway Admin 里的 calls、traces、logs、health、stats 和 Token 活动不是装饰面板。
它们让团队看到 Agent 实际选择了什么工具、参数是否通过、失败发生在哪一层、哪些
Skill 被频繁调用。发现重复失败后，可以修改说明、schema 或实现，再用真实调用数据
验证效果，形成工具和 Skill 的迭代闭环。

我们也保留明确边界：Core 不能安全抢占已经在 DCC 主线程执行的任意宿主代码，不能替
没有事务 API 的 DCC 保证回滚，也不会假装存在一套对所有软件都无损的
mesh/rig/material 模型。宿主语义仍应由 adapter 和 pipeline Skill 负责。

如果你也希望 DCC Agent 从 Demo 走向可复用、可部署、可观察的生产工具，欢迎试用并给
项目一个 Star。
