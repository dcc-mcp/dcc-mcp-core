...
- **Workspace 包成员**以根目录 `Cargo.toml` 为准；本页只描述 crate 边界，不重复容易过期的包数量。
...
├── dcc-mcp-gateway-admin # Admin trace/caller/projection 领域和可选嵌入式 dashboard 资产
...
- Admin/dashboard — `/admin/api/*` 只读检查 instances、tools、calls、traces、stats、workers、logs、health。Trace、caller context、token telemetry、有界 trace log 与面向 agent 的 compact projection 契约由 `dcc-mcp-gateway-admin` 持有；gateway crate 保留兼容导入路径。
