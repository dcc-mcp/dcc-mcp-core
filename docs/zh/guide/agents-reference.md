...
`verified=false`；不得把它当作没有副作用来重试。Core 不会自动注册或宣称 `execute_python` 路由；
适配器必须先注册 provider 才能暴露该路由，并在 `capture_state_digest(...)` 报告 provider 缺失时，
把 `unavailable/provider_missing` 能力状态写入 discovery 元数据。
