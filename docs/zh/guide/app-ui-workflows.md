...
可见角标、底部胶囊和鼠标效果属于适配器宿主的交互式 Windows 登录会话。如果用户按下 `Ctrl+Alt+Esc` 并收到 `user_interrupted`，请立即停止。普通 `Esc` 仍会发送给目标 DCC，用于取消工具或关闭对话框。不要重试、改变 `session_id` 或启动新会话。只有用户明确要求恢复后，才能设置 `resume_computer_use=true`。
