...
核心库支持 Python 3.7–3.14。Python 3.7 在 Linux 和 Windows 上使用原生 `cp37-cp37m` wheel，其他平台使用 `py37-lite` sidecar 回退。详见 [Python 3.7 LTS 策略](/zh/guide/maya2022-support)。
...
Python 3.7–3.14 都有 CI 覆盖。Python 3.7 具备原生 wheel、运行时与完整测试门禁；Python 3.8+ 使用 `abi3-py38` 和维护版本矩阵。
