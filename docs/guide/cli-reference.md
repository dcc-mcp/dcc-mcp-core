...
| `feedback file <finding.json> [--catalog <path>] [--existing <number>\|--create] [--yes] [--json]` | local public-safe Finding + GitHub CLI | Route and deduplicate a Finding against open issues. The default is read-only and returns an executable `next_step`; an external write requires both `--yes` and an explicit `--existing` or `--create` decision. Exact fingerprint conflicts, ambiguous keyword matches, closed issues, and tracker failures stop the operation. |
...
7. Run `dcc-mcp-cli feedback file finding.json --json` to search the routed
   ...
   `--existing <number> --yes` next step; no match returns a `--create --yes`
   next step. Keyword-only or multiple matches return review options and must
   not be auto-selected. Execute a reviewed argv only with user authorization.
   The command repeats exact-fingerprint search immediately before any comment
   or create operation and fails closed if the result changed.
