...
| - ClawHub/OpenClaw skills: dcc-mcp                                      |
...
- **Workspace packages** are defined by the root `Cargo.toml`; keep this guide focused on crate boundaries rather than duplicating a volatile package count.
...
├── dcc-mcp-gateway-admin # Admin trace/caller/projection domain + optional embedded dashboard asset
...
**Dependencies**: Domain serialization/identity helpers, the transport-neutral `dcc-mcp-jsonrpc` wire types, and `dcc-mcp-gateway-search`; no Axum, Reqwest, Tokio, registry, or admin dependency.
