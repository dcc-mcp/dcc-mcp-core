...
`production-tracking` returns records that carry both tags. Use `tags_any` when
any tag is acceptable (e.g. `{"tags_any":["shotgrid","ftrack"]}`). Use
`dcc_types` when the same search should span several backend families.
