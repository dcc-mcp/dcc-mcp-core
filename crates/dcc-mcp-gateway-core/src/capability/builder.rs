...
//!
//! `dcc-mcp-jsonrpc` provides transport-neutral MCP wire types only;
//! it has no HTTP or async-runtime dependency. Keeping the complete,
//! deterministic wire-to-domain translation here lets every gateway
//! runtime share one implementation without pulling in infrastructure.

use serde_json::{Value, json};
use uuid::Uuid;

use crate::capability::index::{InstanceFingerprint, compute_fingerprint};
use crate::capability_naming::{
...
pub struct BuildOutcome {
    ...
    /// Stable fingerprint of the input tool list.
    pub fingerprint: InstanceFingerprint,
    /// Number of rejected input tools.
    pub skipped: usize,
}

const BACKEND_JOB_STATUS_TOOL: &str = "jobs_get_status";

/// Return whether `name` identifies the adapter-owned async job status tool.
#[must_use]
pub fn is_backend_job_tool(name: &str) -> bool {
    name == BACKEND_JOB_STATUS_TOOL
}

/// Build the synthetic MCP descriptor for the adapter-owned job status tool.
#[must_use]
pub fn backend_job_status_tool() -> McpTool {
    McpTool {
