...
-- inserting. That bound is NOT self-enforcing, though. The `fingerprint` on a
-- finding is supplied by the caller and the server only checks its *shape*
-- (`sha256:` + 64 lowercase hex digits, see `FindingV1::validate`); the server
-- does not recompute it, so a caller can mint arbitrarily many distinct values
-- and each one lands a new row. Verifying the digest server-side is not
-- possible on this path either: `finding_fingerprint` takes the owning repo as
-- its first input, and that repo is exactly what routing has to derive, so
-- trusting the client-supplied fingerprint is the existing Finding v1
-- contract.
--
-- The only bound in effect today is therefore the optional per-IP rate limit
-- on the gateway ingress (`rate_limit_per_minute_per_ip`); with it disabled, a
-- caller can add rows without limit. If ingest volume ever makes row count a
-- concern, cap by `last_seen_ms` or evict low-`occurrence_count` rows rather
-- than pruning on age alone.
