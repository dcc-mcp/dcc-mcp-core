...
/// * `max_total_size_mb` — when the total size of eligible `.log` files matching
...
///
/// A trailing numeric process id is removed from the matching scope so a new
/// process can collect logs left by earlier processes with the same base
/// prefix. Files from today and files owned by a live process are never
/// deleted.
pub fn prune_old_logs(directory: &Path, prefix: &str, retention_days: u32, max_total_size_mb: u32) {
    if retention_days == 0 && max_total_size_mb == 0 {
        return;
    }
    ...
    let (scope, process_scoped) = prefix
        .rsplit_once('.')
        .filter(|(scope, suffix)| scope.starts_with("dcc-mcp-") && suffix.parse::<u32>().is_ok())
        .map_or((prefix, false), |(scope, _)| (scope, true));
    ...
    let mut files: Vec<(std::time::SystemTime, u64, PathBuf, Option<u32>)> = Vec::new();
    ...
    for entry in read_dir.flatten() {
        ...
        if suffix.starts_with(&today) {
            continue;
        }
        let pid = if process_scoped {
            let Some((pid, dated)) = suffix.split_once('.') else {
                continue;
            };
            if dated.starts_with(&today) {
                continue;
            }
        ...
        files.push((modified, size, path, pid));
    }

    let mut system = System::new();
    let liveness_available = !process_scoped
        || files.is_empty()
        || system.refresh_processes(ProcessesToUpdate::All, true) > 0;

    let mut retained: Vec<(std::time::SystemTime, u64, PathBuf)> = Vec::new();
    let mut total_size: u64 = 0;
    for (modified, size, path, pid) in files {
        ...
        total_size += size;
        retained.push((modified, size, path));
    }
