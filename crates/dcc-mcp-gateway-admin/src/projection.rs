...
/// Build the bounded agent-facing summary for an admin debug bundle.
#[must_use]
pub fn compact_debug_bundle_payload(bundle: &Value) -> Value {
...
/// Build the bounded agent-facing summary for one dispatch trace.
#[must_use]
pub fn compact_trace_detail_payload(trace: &Value) -> Value {
    json!({
...
/// Build compact summaries for an admin trace-list response.
#[must_use]
pub fn compact_trace_list_payload(payload: &Value) -> Value {
...
/// Build a compact trace-context response around one selected trace.
#[must_use]
pub fn compact_trace_context_payload(payload: &Value) -> Value {
    json!({
...
mod tests {
    ...
    #[test]
    fn debug_bundle_projection_omits_payload_content_and_classifies_root_cause() {
        let bundle = json!({
            "request_id": "req-1",
            "trace": {
                "tool_slug": "maya.render",
                "dcc_type": "maya",
                "payload": {
                    "mime_type": "application/json",
                    "original_size": 4096,
                    "truncated": true,
                    "content": "[REDACTED] scene.ma"
                }
            },
            "hints": ["backend unavailable"]
        });

        let compact = compact_debug_bundle_payload(&bundle);

        assert_eq!(compact["request_id"], "req-1");
        assert_eq!(compact["root_cause"], "host died");
        assert_eq!(compact["redaction"]["payload_preview_count"], 1);
        assert_eq!(compact["redaction"]["truncated_payload_count"], 1);
        assert_eq!(compact["redaction"]["redacted_marker_count"], 1);
        assert!(compact.get("trace").is_none());
        assert!(!compact.to_string().contains("scene.ma"));
    }

    #[test]
    fn trace_collection_projections_preserve_counts_and_selected_trace() {
        let payload = json!({
            "total": 2,
            "traces": [
                {"request_id": "req-1", "ok": true, "method": "call"},
                {"request_id": "req-2", "ok": false, "method": "call"}
            ],
            "links": {"next": "/admin/api/traces?page=2"}
        });

        let list = compact_trace_list_payload(&payload);
        assert_eq!(list["total"], 2);
        assert_eq!(list["traces"][0]["status"], "ok");
        assert_eq!(list["traces"][1]["status"], "err");

        let context = compact_trace_context_payload(&json!({
            "lookup_id": "trace-1",
            "trace": payload["traces"][0],
            "traces": payload["traces"]
        }));
        assert_eq!(context["lookup_id"], "trace-1");
        assert_eq!(context["trace_count"], 2);
        assert_eq!(context["trace"]["request_id"], "req-1");
    }
}
