...
/// Selects whether an exported issue report is safe for public attachment or retains raw evidence.
pub enum IssueReportMode {
    /// Redact private payload material and emit a public-safe diagnostic summary.
    PublicSafe,
    /// Preserve the correlated debug bundle for explicit local review.
    RawDebugBundle,
}

/// Build the stable attachment filename for an issue-report request identifier.
#[must_use]
pub fn issue_report_filename(request_id: &str) -> String {
...
/// Build an issue-report payload using the requested privacy contract.
#[must_use]
pub fn issue_report_json(
    request_id: &str,
    bundle: Value,
    links: Value,
...
mod tests {
    ...
    fn private_bundle() -> Value {
        json!({
            "trace_id": "trace-maya-1",
            "trace": {
                "tool_slug": "maya.scene__open_file",
                "dcc_type": "maya",
                "ok": false,
                "total_ms": 125,
                "error": "host died while opening C:\\studio\\secret.ma",
                "input": {
                    "estimated_tokens": 4,
                    "preview": "C:\\studio\\secret.ma"
                }
            },
            "postmortem": {
                "gateway_events": [{"status": "host_died"}]
            }
        })
    }

    #[test]
    fn public_safe_report_excludes_raw_bundle_and_private_evidence() {
        let report = issue_report_json(
            "maya request/1",
            private_bundle(),
            json!({"private_url": "http://127.0.0.1/admin"}),
            IssueReportMode::PublicSafe,
        );

        assert_eq!(report["privacy_mode"], "public-safe");
        assert_eq!(report["summary"]["error"]["kind"], "backend-unavailable");
        assert_eq!(report["summary"]["payload_tokens"]["input_tokens"], 4);
        assert!(report.get("debug_bundle").is_none());
        let encoded = serde_json::to_string(&report).unwrap();
        for private in ["127.0.0.1", "C:\\\\studio", "secret.ma"] {
            assert!(!encoded.contains(private), "public report leaked {private}");
        }
        assert_eq!(
            report["links"]["safe_issue_report_path"],
            "/admin/api/issue-report/maya%20request%2F1"
        );
    }

    #[test]
    fn raw_report_preserves_explicit_local_evidence_and_links() {
        let report = issue_report_json(
            "req-raw",
            private_bundle(),
            json!({"trace_api_url": "http://127.0.0.1/admin/api/traces/req-raw"}),
            IssueReportMode::RawDebugBundle,
        );

        assert_eq!(report["privacy_mode"], "raw-local-evidence");
        assert_eq!(report["debug_bundle"]["trace_id"], "trace-maya-1");
        assert_eq!(
            report["links"]["trace_api_url"],
            "http://127.0.0.1/admin/api/traces/req-raw"
        );
    }

    #[test]
    fn attachment_filename_sanitizes_path_separators_and_empty_identifiers() {
        assert_eq!(
            issue_report_filename("maya/request:1"),
            "dcc-mcp-issue-report-maya-request-1.json"
        );
        assert_eq!(
            issue_report_filename(""),
            "dcc-mcp-issue-report-request.json"
        );
    }
}
