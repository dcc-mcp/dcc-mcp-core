...
//! calls `TraceStatsAggregator::compute()` on the current ring-buffer snapshot;
...
use crate::{AgentContext, DispatchTrace, TOKEN_ESTIMATOR, TokenTelemetry, TraceLog};
...
pub struct TraceStatsAggregator {
    trace_log: Arc<TraceLog>,
}

impl TraceStatsAggregator {
    ...
    pub fn compute_filtered(&self, range: StatsRange, filter: &StatsFilter) -> GatewayStats {
        compute_stats_filtered(self.trace_log.recent(usize::MAX), range, filter)
    }
}

/// Select, order, and aggregate a merged set of trace records.
#[must_use]
pub fn compute_stats_filtered(
    traces: Vec<DispatchTrace>,
    range: StatsRange,
    filter: &StatsFilter,
) -> GatewayStats {
    let cutoff = range.cutoff();
    let mut traces: Vec<DispatchTrace> = traces
        .into_iter()
        .filter(|trace| {
            cutoff
                .map(|value| trace.started_at >= value)
                .unwrap_or(true)
        })
        .filter(|trace| filter.matches(trace))
        .collect();
    traces.sort_by(|left, right| {
        let left_started = left.started_at.duration_since(UNIX_EPOCH).ok();
        let right_started = right.started_at.duration_since(UNIX_EPOCH).ok();
        right_started.cmp(&left_started)
    });
    compute_stats(&traces, range)
}
...
fn compute_stats(in_range: &[DispatchTrace], range: StatsRange) -> GatewayStats {
...
mod tests {
    ...
    use super::*;
    use crate::{AgentContext, DispatchTrace, TokenTelemetry, TraceLog, TracePayload};
    use serde_json::json;
    ...
    fn empty_log_returns_zero_stats() {
        ...
        let agg = TraceStatsAggregator::new(log);
    ...
    fn token_usage_aggregates_totals_and_breakdowns() {
        ...
        let stats = TraceStatsAggregator::new(log).compute(StatsRange::All);
