...
impl FileRegistry {
    ...
    fn read_entries_from_file(&self) -> TransportResult<Vec<ServiceEntry>> {
        ...
        match parse_registry_entries(&content) {
            ...
            Err(RegistryParseError::Json(e)) => {
                self.quarantine_corrupted_registry_file(&path, &e);
                Ok(Vec::new())
            }
            Err(RegistryParseError::UnsupportedSchema {
                received,
                supported,
            }) => {
                tracing::warn!(
                    ...
                    received,
                    supported,
                    "FileRegistry: registry schema is newer than this build; preserving the file without loading it"
                );
