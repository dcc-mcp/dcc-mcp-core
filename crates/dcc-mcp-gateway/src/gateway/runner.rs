...
const RECOVERY_BIND_ATTEMPTS: u32 = 3;
...
fn service_dead_port_holder_pid(
    port: u16,
    registry_dir: &Path,
    pidfile: Option<&Path>,
) -> Option<u32> {
...
async fn reap_service_dead_port_holder(
    host: &str,
    port: u16,
    pid: u32,
    ...
    loop {
        if super::bind::try_bind_port_opt(host, port).await.is_some() {
            return true;
        }
