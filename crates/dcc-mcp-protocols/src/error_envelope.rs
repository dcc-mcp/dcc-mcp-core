...
//! [`ToolCallErrorEnvelope`] so that both agents and humans can programmatically
...
pub struct ToolCallErrorEnvelope {
...
/// Compatibility name for the MCP `tools/call` wire envelope.
///
/// This is not the cross-crate domain error enum owned by
/// `dcc-mcp-models`. New code should use [`ToolCallErrorEnvelope`] so the
/// wire projection cannot be confused with `dcc_mcp_models::DccMcpError`.
#[deprecated(
    since = "0.20.9",
    note = "use ToolCallErrorEnvelope; dcc_mcp_models::DccMcpError is the domain error"
)]
pub type DccMcpError = ToolCallErrorEnvelope;

impl ToolCallErrorEnvelope {
...
impl std::fmt::Display for ToolCallErrorEnvelope {
...
/// Well-known layer names for [`ToolCallErrorEnvelope::layer`].
pub mod layers {
...
/// Well-known error codes for [`ToolCallErrorEnvelope::code`].
pub mod codes {
...
mod tests {
    ...
    fn test_error_serialize_roundtrip() {
        let err =
            ToolCallErrorEnvelope::new("gateway", "SKILL_NOT_LOADED", "Skill 'x' is not loaded.")
        ...
        let parsed: ToolCallErrorEnvelope = serde_json::from_str(&json).unwrap();
    ...
    fn test_new_auto_generates_trace_id() {
        let err = ToolCallErrorEnvelope::new("gateway", "SKILL_NOT_LOADED", "Not loaded");
        assert!(err.trace_id.is_some());
    ...
    fn test_trace_ids_are_unique() {
        let e1 = ToolCallErrorEnvelope::new("gateway", "A", "a");
        let e2 = ToolCallErrorEnvelope::new("gateway", "A", "a");
    ...
    fn test_display_without_hint() {
        let err = ToolCallErrorEnvelope::new("instance", "EXECUTION_FAILED", "Handler panicked");
        let s = format!("{err}");
    ...
    #[test]
    fn test_pretty_json() {
        let err = ToolCallErrorEnvelope::new("dcc", "DCC_CMD_FAILED", "Command failed")
