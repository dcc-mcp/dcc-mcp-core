"""Regression coverage for #1133: ui_control through gateway search/describe/call."""
...
@pytest.fixture()
def ui_control_gateway(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    registry_dir = tmp_path / "registry"
    registry_dir.mkdir()
    state_dir = tmp_path / "ui-control-state"
    monkeypatch.setenv("DCC_MCP_UI_CONTROL_BACKEND", "mock")
    monkeypatch.setenv("DCC_MCP_UI_CONTROL_MOCK_STATE_DIR", str(state_dir))
    ...
    cfg = McpHttpConfig(port=0, server_name="ui-control-backend")
...
def _load_ui_control(gateway_mcp_url: str) -> None:
    resp = _post_mcp(
        gateway_mcp_url,
        "tools/call",
        {"name": "load_skill", "arguments": {"skill_name": "ui-control"}},
    )
...
def test_ui_control_gateway_rest_and_mcp_discovery_describe_call(ui_control_gateway: dict) -> None:
    gateway_mcp_url = ui_control_gateway["gateway_mcp_url"]
    gateway_rest_url = ui_control_gateway["gateway_rest_url"]
    _load_ui_control(gateway_mcp_url)

    search_resp = _post_mcp(
        gateway_mcp_url,
        "tools/call",
        {"name": "search_tools", "arguments": {"query": "ui_control snapshot", "dcc_type": "python"}},
    )
    assert "error" not in search_resp, search_resp.get("error")
    assert "ui_control__snapshot" in _tool_text(search_resp)

    slug = _find_ui_control_slug(gateway_rest_url)
