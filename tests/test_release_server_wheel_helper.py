"""Server-wheel Windows companion injection tests."""
...
from scripts.release.inject_capture_helper import inject_helpers
...
def test_inject_helpers_place_both_companions_and_rewrite_record(tmp_path: Path) -> None:
    ...
    inject_helpers(wheel, helper, host)
...
def test_inject_helpers_reject_wheel_without_server_script(tmp_path: Path) -> None:
    wheel = tmp_path / "dcc_mcp_server-1.0.0-cp311-none-win_amd64.whl"
    helper = tmp_path / "dcc-mcp-capture-helper.exe"
    helper.write_bytes(b"MZhelper")
    host = tmp_path / "dcc-mcp-ui-control-host.exe"
