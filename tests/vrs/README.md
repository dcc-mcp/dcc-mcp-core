...
| `traces/core-2208-gateway-feedback.jsonl` | No | Gateway accepts correlated crash feedback even when the referenced DCC instance is already gone. |
...
| `traces/core-996-instance-offline-carries-previous-status.jsonl` | No | `instance-offline` (or `unknown-slug`) carries lifecycle provenance and actionability; never-registered is 404 while known exited owners are 410, not a generic 503 (core#996). |
...
| `traces/core-async-job-polling.jsonl` | Yes (any live DCC) | Adapter-owned `jobs_get_status` remains searchable and returns HTTP 200 through `/v1/call`; transient host reload readiness must not turn polling into 502. |
...
- core [#1134](https://github.com/dcc-mcp/dcc-mcp-core/issues/1134) — ui_control DCC debugging workflow examples and REST traces
...
- core [#1659](https://github.com/dcc-mcp/dcc-mcp-core/issues/1659) — gateway load_skill must index loaded skill tools after backend refresh failures
