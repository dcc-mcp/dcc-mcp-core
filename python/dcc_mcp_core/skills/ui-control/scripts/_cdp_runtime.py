"""CDP runtime helpers for ui_control script backends."""
...
def cdp_preset() -> str:
    raw = (
        os.environ.get("DCC_MCP_UI_CONTROL_CDP_PRESET") or os.environ.get("DCC_MCP_UI_CONTROL_CHROME_PRESET") or "reuse"
    )
    ...
    raise CdpBackendError(
        f"Unsupported ui_control CDP preset {raw!r}; use reuse, isolated, auroraview, edge, or agent-browser"
    )


def endpoint_candidates(preset: str) -> List[str]:
    candidates: List[str] = []
    url_names = ["DCC_MCP_UI_CONTROL_CDP_URL"]
    if preset == "auroraview":
        url_names.append("DCC_MCP_UI_CONTROL_AURORAVIEW_CDP_URL")
    elif preset == "edge":
        url_names.append("DCC_MCP_UI_CONTROL_EDGE_CDP_URL")
    elif preset == "agent-browser":
        url_names.append("DCC_MCP_UI_CONTROL_AGENT_BROWSER_CDP_URL")
    else:
        url_names.append("DCC_MCP_UI_CONTROL_CHROME_CDP_URL")
    ...
    if preset == "auroraview":
        port_names = (
            "DCC_MCP_UI_CONTROL_AURORAVIEW_CDP_PORT",
            "AURORAVIEW_CDP_PORT",
            "DCC_MCP_UI_CONTROL_CDP_PORT",
        )
        default_port: Optional[int] = _DEFAULT_CDP_PORT
    elif preset == "reuse":
        port_names = (
            "DCC_MCP_UI_CONTROL_CDP_PORT",
            "DCC_MCP_UI_CONTROL_CHROME_CDP_PORT",
        )
        default_port = _DEFAULT_CDP_PORT
    elif preset == "edge":
        port_names = (
            "DCC_MCP_UI_CONTROL_EDGE_CDP_PORT",
            "EDGE_CDP_PORT",
            "DCC_MCP_UI_CONTROL_CDP_PORT",
        )
        default_port = _DEFAULT_CDP_PORT
        ...
        port_names = (
            "DCC_MCP_UI_CONTROL_AGENT_BROWSER_CDP_PORT",
            "AGENT_BROWSER_CDP_PORT",
            "DCC_MCP_UI_CONTROL_CDP_PORT",
        )
        default_port = None
    else:
        port_names = ("DCC_MCP_UI_CONTROL_CDP_PORT",)
        default_port = None
...
def ensure_cdp_target(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if preset == "auroraview":
        raise CdpBackendError(
            ...
            f"AURORAVIEW_CDP_PORT={_DEFAULT_CDP_PORT} or set DCC_MCP_UI_CONTROL_CDP_URL"
        )
    if preset == "edge":
...
def _find_edge() -> str:
    env_path = os.environ.get("DCC_MCP_UI_CONTROL_EDGE_PATH") or os.environ.get("DCC_MCP_EDGE_PATH")
    candidates = [
        env_path,
    ...
    for candidate in candidates:
        if candidate and Path(candidate).exists():
            return str(candidate)
    raise CdpBackendError("Microsoft Edge executable not found; set DCC_MCP_UI_CONTROL_EDGE_PATH")
...
def _launch_edge(state: Dict[str, Any]) -> Dict[str, Any]:
    edge = _find_edge()
    port = _env_int("DCC_MCP_UI_CONTROL_EDGE_CDP_PORT") or _env_int("DCC_MCP_UI_CONTROL_CDP_PORT") or _free_port()
    user_data = os.environ.get("DCC_MCP_UI_CONTROL_EDGE_USER_DATA_DIR", "")
    profile_dir = os.environ.get("DCC_MCP_UI_CONTROL_EDGE_PROFILE_DIRECTORY", "")
    args = [
        edge,
    ...
    raise CdpBackendError(
        ...
        f"with --remote-debugging-port={_DEFAULT_CDP_PORT} or set DCC_MCP_UI_CONTROL_CDP_URL"
    )
...
def _ensure_agent_browser(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    open_url = os.environ.get("DCC_MCP_UI_CONTROL_AGENT_BROWSER_OPEN_URL", "about:blank")
    _run_agent_browser(["open", open_url], timeout=30.0)
    deadline = time.monotonic() + float(os.environ.get("DCC_MCP_UI_CONTROL_AGENT_BROWSER_WAIT_SECS", "10.0"))
    while time.monotonic() < deadline:
        ws_url = _agent_browser_cdp_url()
        if ws_url:
            connected = _try_connect_endpoint(state, ws_url, "agent-browser")
    ...
    raise CdpBackendError(
        ...
        "run `agent-browser install`, or set DCC_MCP_UI_CONTROL_AGENT_BROWSER_CDP_URL"
    )


def _agent_browser_bin() -> str:
    explicit = (
        os.environ.get("DCC_MCP_UI_CONTROL_AGENT_BROWSER_BIN")
...
def _launch_chrome(state: Dict[str, Any], isolated: bool) -> Dict[str, Any]:
    chrome = _find_chrome()
    port = _env_int("DCC_MCP_UI_CONTROL_CDP_PORT") or _free_port()
    args = [
        chrome,
        f"--remote-debugging-port={port}",
    ]
    if isolated:
        user_data = state.get("user_data_dir") or tempfile.mkdtemp(prefix="dcc-ui-control-chrome-profile-")
        args.append(f"--user-data-dir={user_data}")
    else:
        user_data = os.environ.get("DCC_MCP_UI_CONTROL_CHROME_USER_DATA_DIR", "")
        profile_dir = os.environ.get("DCC_MCP_UI_CONTROL_CHROME_PROFILE_DIRECTORY", "")
        if user_data:
            args.append(f"--user-data-dir={user_data}")
        if profile_dir:
            args.append(f"--profile-directory={profile_dir}")
    ...
    raise CdpBackendError(
        ...
        "DCC_MCP_UI_CONTROL_CDP_URL, or use DCC_MCP_UI_CONTROL_CDP_PRESET=isolated"
    )
