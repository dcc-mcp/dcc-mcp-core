...
def _record_operation(name: str, params: Dict[str, Any], result: Dict[str, Any]) -> None:
    ...
    try:
        ...
        detail = [
            f"backend={os.environ.get('DCC_MCP_APP_UI_BACKEND', 'mock')}",
            f"action={action}",
        ...
        event = {
            ...
            "tool": f"app_ui__{operation}",
            "dcc_type": os.environ.get("DCC_MCP_UI_CONTROL_DCC_TYPE") or os.environ.get("DCC_MCP_DCC_TYPE", "app-ui"),
            "session_id": str(params.get("session_id", "default")),
            "backend": os.environ.get("DCC_MCP_APP_UI_BACKEND", "mock"),
            "action": action,
            "success": success,
            "error": error,
        ...
        line = f"{timestamp} {level} dcc_mcp_core.app_ui.audit: {json.dumps(event, sort_keys=True)}\n"
        with _AUDIT_LOCK, (directory / f"dcc-mcp-app-ui.{os.getpid()}.log").open("a", encoding="utf-8") as stream:
            stream.write(line)
    except Exception:
