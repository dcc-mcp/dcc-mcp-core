...
def _pipe_path() -> str:
    return rf"\\.\pipe\dcc-mcp-ui-control-host-v1-session-{_windows_session_id()}"
...
class UiControlHostClient:
    ...
    def execute(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self._latest_observation_id or not self._latest_accessibility_state_id:
            raise UiControlHostError("stale_observation", "Take a fresh app_ui snapshot before acting.")
        try:
            return self._call(
                {
        ...
        with self._lock:
            try:
                ...
                if not 0 < length <= _MAX_FRAME_BYTES:
                    raise UiControlHostError(
                        "backend_unavailable", "The UI Control host returned an invalid frame length."
                    )
                response = json.loads(_read_exact(self._stream, length).decode("utf-8"))
