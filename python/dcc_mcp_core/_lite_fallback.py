"""Pure-Python fallbacks for environments without the compiled ``_core``.

When ``dcc_mcp_core._core`` is available (full Rust build), this module
re-exports the compiled versions. When ``_core`` is absent (for example, in a
lite wheel on any Python version), it provides pure-Python implementations of ``parse_skill_md``,
